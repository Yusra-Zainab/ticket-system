...
import { addClientActivity, getClientTicketAccess } from "@/lib/clientPortal";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!user || !isClientRole(user.role)) return Response.json({ error: "Authentication required." }, { status: 401 });
    ...
    for (const file of files) {
      ...
      uploaded.push({ id: attachmentId, name: file.name, url: `/api/client-portal/attachments/${attachmentId}` });
    }
    await addClientActivity(access.databaseId, user.id, `Uploaded ${uploaded.length} attachment${uploaded.length === 1 ? "" : "s"}`, access.status);
