  ...
  return (
    <header className="cp-page-header">
      <div className="cp-breadcrumb-row">
        <nav className="cp-breadcrumbs" aria-label="Breadcrumbs">
          <Link href="/client/dashboard" className="cp-crumb-icon" aria-label="Dashboard"><Home size={18} /></Link>
          {crumbs.map((crumb) => (
            <span className="cp-crumb-part" key={`${crumb.label}-${crumb.href ?? "current"}`}>
              <ChevronRight size={15} />
              {crumb.href ? <Link href={crumb.href}>{crumb.label}</Link> : <span className="cp-crumb-current">{crumb.label}</span>}
            </span>
          ))}
        </nav>
        <div className="cp-history-controls">
          ...
          <button type="button" onClick={() => { setRefreshing(true); router.refresh(); setTimeout(() => setRefreshing(false), 800); }} aria-label="Refresh">
            <RotateCw size={17} className={refreshing ? "cp-spin" : ""} />
          </button>
        </div>
      </div>
      <div className="cp-title-row">
        <h1>{title}</h1>
        {actionHref && actionLabel ? <Link className="cp-primary-button" href={actionHref}><Plus size={19} />{actionLabel}</Link> : null}
      </div>
    </header>
  );
}
