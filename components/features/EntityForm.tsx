...
const schemas = {
  ...
  resource: z.object({ name: z.string().min(2), email: z.email(), phone: z.string().min(7), role: z.string().min(2), skills: z.string().min(2), capacity: z.coerce.number().min(1).max(100) }),
};
type Kind = keyof typeof schemas;
type Values = Record<string, string | number | string[]>;

export default function EntityForm({ kind }: { kind: Kind }) {
  const router = useRouter(); const [step, setStep] = useState(0); const [loading, setLoading] = useState(false); const [submitError, setSubmitError] = useState('');
  const { register, control, formState: { errors }, handleSubmit } = useForm({ resolver: zodResolver(schemas[kind]), defaultValues: kind === 'project' ? { name: '', client: '', description: '', startDate: '', dueDate: '', budget: 0, team: [] } : kind === 'client' ? { name: '', company: '', email: '', phone: '', notes: '' } : { name: '', email: '', phone: '', role: '', skills: '', capacity: 75 } });
  ...
  const projectSteps = [
    ...
    { id: 'team', title: 'Assign team', description: 'Select delivery owners', component: <fieldset><legend className="label mb-3">Team members</legend><div className="grid gap-3 sm:grid-cols-2">{mockUsers.map((user) => <label key={user.id} className="flex items-center gap-3 rounded-xl border border-slate-200 p-4 hover:bg-slate-50"><input type="checkbox" value={user.name} {...register('team')} className="checkbox" /><span><span className="block text-sm font-semibold text-slate-800">{user.name}</span><span className="text-xs text-slate-400">{user.role}</span></span></label>)}</div><span className="error-text">{fieldErrors.team?.message}</span></fieldset> },
  ...
  const resourceSteps = [
    { id: 'identity', title: 'Profile', description: 'Identity and contact', component: <div className="grid gap-5 sm:grid-cols-2">{field('name', 'Full name')}{field('email', 'Email', 'email')}{field('phone', 'Phone', 'tel')}{field('role', 'Role')}</div> },
  ...
  const submit = handleSubmit(async (values) => { try { setLoading(true); setSubmitError(''); await api.submit(kind, values as Values); router.push(`/${kind === 'project' ? 'projects' : kind === 'client' ? 'clients' : 'resources'}`); } catch (error) { setSubmitError(error instanceof Error ? error.message : 'Unable to save. Please try again.'); } finally { setLoading(false); } });
