...
export default function ResourceTicketForm({
  ...
  const [type, setType] = useState<ResourceTicketType>(initialType);
  ...
  const createdBy = initialTicket?.reporter || "Current resource";
  ...
  function jump(id: SectionId) {
    setActive(id);
    document
      .getElementById(`resource-ticket-${id}`)
  ...
  async function uploadPendingFiles(ticketId: string) {
    ...
    try {
      ...
      const response = await fetch(
        `/api/resource-portal/tickets/${encodeURIComponent(ticketId)}/attachments`,
          ...
          >
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label="Assigned To">
                {selectedProject?.allowSelfAssign ? (
                  <select
                    value={selfAssign ? "self" : ""}
                    onChange={(event) => setSelfAssign(event.target.value === "self")}
                    className="field"
                  >
                    <option value="">Project team will assign</option>
                    <option value="self">Assign to me</option>
                  </select>
                ) : (
                  <input
                    value="Project team will assign"
                    readOnly
                    className="field !bg-slate-50"
                  />
                )}
          ...
          >
            <Field label="Notes">
              <div className="field !min-h-32 !bg-slate-50 leading-6 text-slate-500">
                Internal notes are restricted to admin and project-management roles.
                Resource users can add public comments after the ticket is created.
              </div>
            </Field>
          </FormSection>
        </main>
